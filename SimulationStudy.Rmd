...
num.obs <- 200 # number of observations
num.par <- 500 # number of parameters
...
for (run in 1:num.runs){
  ...
  # Perform the Oracle pipeline
  cat("  -> compute Oracle tuning parameters... ")
...
output.Data <- matrix(c(
  mean(errors.pav), mean(errors.oracle), mean(errors.fridge), mean(errors.CV10), mean(errors.CV5),
  sd(errors.pav), sd(errors.oracle), sd(errors.fridge), sd(errors.CV10), sd(errors.CV5),
  compTime.pav.scaled, "None",  compTime.fridge.scaled, compTime.CV10.scaled, compTime.CV5.scaled),
  nrow = 5, ncol = 3)
rownames(output.Data) <- c("Pav.edr", "Oracle", "Fridge", "10-fold CV", "5-fold CV")
